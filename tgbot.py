...
from telebot import editMessageReplyMarkup
reload(sys)
...
def MainMenu(cid):
    Menu = types.ReplyKeyboardMarkup(one_time_keyboard = True, row_width = 2)
    ...
    for row in myresult:
        for val in range(2):
            if val == 0:
                ...
                msg2 = row[val]
        markup.add(types.InlineKeyboardButton("Appeal num [" + str(msg1) + "] written on " + str(msg2), callback_data=str(msg1)))
...
def command_help(m):
    cid = m.chat.id
    sql = "select case when exists (select user_id from users where user_id = %s) then 1 else 0 end;"
    val = (cid,)
    mycursor.execute(sql,val)
    myresult = mycursor.fetchall()
    print(str(myresult))
...
def appealCheck(m):
    ...
    text = m.text
    if isDigit(text):
        sql = "select case when exists (select appeal_id from appeal_info where appeal_id = %s and user_id = %s) then 1 else 0 end;"
        val = (text, cid, )
        mycursor.execute(sql, val)
        myresult = mycursor.fetchall()
        for row in myresult:
        ...
        if ans == 1:
            ...
            bot.send_message(cid, "Вашего заявления нет в базе, перепроверьте или обратитесь в тех. поддержку 109")
    else:
        bot.send_message(cid, "Вы ввели данные в неверном формате!\nПожалуйста ввведите только номер обращения")
...
def callback(call):
    if call.data == "back":
        time.sleep(1)
        bot.editMessageReplyMarkup(call.message.chat.id, call.inline_message_id,reply_markup = None)
        MainMenu(call.message.chat.id)
        userStep[call.message.chat.id] = 10
    else:
        ...
        bot.answer_callback_query(callback_query_id=call.id, show_alert=True,
                              text="Ваше обращение под номером # " + str(id) + " имеет статус " + str(status))
